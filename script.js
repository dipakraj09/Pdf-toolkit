...
pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
        ...
        async function processFiles(files) {
            ...
            const THUMB_SCALE = 2;
            const THUMB_Q     = 1;
            const BATCH       = 2;   // keep memory use stable for large PDFs
        ...
        function applyFeatureVisibility() {
            ...
            demoTitle.classList.toggle('demo-trigger',appSettings.demoPdf);
            demoTitle.querySelector('.demo-badge').style.display=appSettings.demoPdf?'inline-block':'none';
            document.querySelector('.preview-edit-btn').style.display=appSettings.imageEditor?'flex':'none';
        ...
        function openMergeRenamePopover(e) {
            ...
            mergeFileNameInput.value=`PDF-Studio-Merged-${selectedItems.size}-Pages`;
        ...
        async function getHDDataUrl(item) {
            ...
            if (item.type === 'image') {
                ...
                const pageNum = item.type === 'pdf-page' ? item.originalPageNum : 1;
                const page = await pdfDoc.getPage(pageNum);
                const hdCanvas = await renderPageToCanvas(page, 2.5);
        ...
        function getItemBaseFileName(item) {
            const i=uploadedItems.findIndex(x=>x.id===item.id);
            const source=String(item.sourceFileName||'document').replace(/(\\.pdf|\\.png|\\.jpg|\\.jpeg)$/i,'').replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'')||'document';
            const kind=item.type==='pdf-page' ? `Page-${String(item.originalPageNum).padStart(2,'0')}` : item.type==='pdf-file' ? 'Full-PDF' : 'Image';
            return `PDF-Studio-${String(i+1).padStart(3,'0')}-${source}-${kind}`;
        }
        ...
        async function mergeSelectedItems(customName=null) {
            ...
            try {
                ...
                const name=customName?`${sanitizePdfFileName(customName)}.pdf`:`PDF-Studio-Merged-${ids.length}-Pages.pdf`;
        ...
        async function quickMergeSelectedItems() {
            ...
            try {
                ...
                downloadFile(pdfBytes, `PDF-Studio-Quick-Merge-${ids.length}-Pages.pdf`, 'application/pdf');
        ...
        function editorStart(p) {
            if(editor.currentTool==='crop'||editor.currentTool==='text'){
                if(editor.currentTool==='text'){createTextBox(p);}
                return;
            }
        ...
        function hideCropUI() { document.getElementById('cropUI').classList.remove('show'); cropState.active=false; }
        ...
        function finalizeAllTextBoxes() {
            ...
            editor.textBoxes.forEach(({box,ce})=>{
                ...
                const x=(br.left-wr.left)*editor.scaleX, y=(br.top-wr.top)*editor.scaleY;
                ...
                ctx.globalCompositeOperation='source-over';
                text.split('\n').forEach((line,i)=>ctx.fillText(line, x+4*editor.scaleX, y+(i*fSize*1.2)+4*editor.scaleY));
                box.remove();
            });
